...
# ✅ Sozhaa Foundation website as knowledge source
COMPANY_URLS = [
    "https://sozhaafoundation.com/",
    "https://sozhaafoundation.com/about",
    "https://sozhaafoundation.com/programs",
    "https://sozhaafoundation.com/projects",
    "https://sozhaafoundation.com/contact",
]

STORAGE_DIR = "chat_data"
TRANSCRIPT_EXCEL = os.path.join(STORAGE_DIR, "sozhaa_foundation_chat_history.xlsx")
TRANSCRIPT_JSON = os.path.join(STORAGE_DIR, "sozhaa_foundation_transcripts.json")
...
def fetch_snippets(urls, chars=1500):
    snippets = []
    headers = {"User-Agent": "SozhaaFoundationBot/1.0 (+https://sozhaafoundation.com)"}
    for u in urls:
        try:
...
def build_system_prompt(snippets):
    ...
    return (
        "You are Sozhaa Foundation AI Assistant. Use only the company's information provided below "
        "(sozhaafoundation.com). Answer only about the foundation, its programs, projects, mission, and contact details. "
        "If asked outside scope, reply politely that you only provide Sozhaa Foundation info. "
...
def build_html_email(user_details, service, transcript):
    header = f"""
    <h2>Sozhaa Foundation — Chat Transcript</h2>
...
@app.get("/")
def root():
    return {"status": "ok", "msg": "Sozhaa Foundation Chatbot backend running"}
...
async def chat_endpoint(payload: ChatPayload, background_tasks: BackgroundTasks):
    ...
    if "[User ended the chat]" in user_msg:
        assistant_text = "✅ Thank you for chatting with Sozhaa Foundation 🚀<br>Our team will contact you soon."
        ...
        if user_phone:
            thank_msg = "✅ Thanks for contacting Sozhaa Foundation. Our team will contact you soon 🚀"
        ...
        if payload.user_details.get("email"):
            thank_you_html = html + "<br><br><p>🙏 Thank you for chatting with Sozhaa Foundation. Our team will connect with you soon.</p>"
            send_email_with_attachment(payload.user_details["email"], "Sozhaa Group — Chat Summary", thank_you_html, TRANSCRIPT_EXCEL)
    ...
    if "support" in user_msg.lower() or "contact" in user_msg.lower():
        ...
        def support_alert():
            try:
                ...
                if payload.user_details.get("email"):
                    send_email_with_attachment(payload.user_details["email"], "Sozhaa Foundation — Support Request Received", "<p>We received your request. Our team will contact you soon 🚀</p>")
            except Exception as e:
    ...
    def save_and_email():
        ...
        if payload.user_details.get("email"):
            send_email_with_attachment(payload.user_details["email"], "Sozhaa Foundation — Your Chat Transcript", html, TRANSCRIPT_EXCEL)
